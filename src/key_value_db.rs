    ...
    ID: Id,
{
    ...
    pub(crate) fn get_changes(
        ...
        for (k, v) in &self.changes_store.current_changes.0 {
            if let TrieKey::Flat(k) = k {
                let old_value = v.old_value.clone();
                let new_value = v.new_value.clone();

                leaf_changes.insert(
                    bytes_to_bitvec(&k),
                    ExternChange {
                        // SAFETY: We are sure that the values are valid Felt because they can be saved only by our crate
                        old_value: old_value.map(|x| Felt::decode(&mut x.as_ref()).unwrap()),
                        new_value: new_value.map(|x| Felt::decode(&mut x.as_ref()).unwrap()),
                    },
                );
            }
        }
